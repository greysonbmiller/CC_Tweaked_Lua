...
-- Link routine: pull from front inventory, push into right inventory
local function linkRoutine()
  print("Executing: Link Routine")

  -- Try to wrap the inventory in front
  local frontInventory = peripheral.wrap("front")
  if not frontInventory then
    print("Error: No inventory found in front")
    return
  end

  -- Wrap the chest on the right side
  local rightInventory = peripheral.wrap("right")
  if not rightInventory then
    print("Error: No inventory found on right side")
    return
  end

  -- Get number of slots in front inventory
  local size = frontInventory.getInventorySize and frontInventory.getInventorySize()
  if not size then
    print("Error: Front block is not a valid inventory")
    return
  end

  local targetSide = "left" -- the inventory behind the turtle (relative to front-facing chest)

  for slot = 1, size do
    local item = frontInventory.getItemDetail(slot)
    if item then
      local moved = frontInventory.pushItems(targetSide, slot, item.count)
