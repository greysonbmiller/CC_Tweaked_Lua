...
-- THIS FILE DIFFERS FROM THE LAPIS TWIN ON PURPOSE (the "allthemodium patch").
-- Allthemodium ore is rare enough to be worth coming to in person, so instead
-- of offering a warp point every 4th cycle on a timer, this bot drops one at
-- EVERY ore it reaches - the warpPlate() call lives at the end of seek(), and
-- the one in the main loop stays commented out.
--
-- It also waits on io.read() rather than a countdown: it holds the warp point
-- open indefinitely until someone arrives and presses Enter on the turtle,
-- because there is no point resuming after 30 seconds if nobody came. That is
-- deliberate, not a hang. The chunk loader keeps it alive while it waits.
...
-- at the top of the file. Deploys the same warp point a distress call does,
-- then holds it open until a human actually turns up.
local function warpPlate()
    ...
    if not plateFace then
        -- Not fatal: the bot is healthy and will hit another ore. Say so and
        -- carry on rather than stopping a working bot over one blocked spot.
        print("Could not place the warp plate here; carrying on.")
        pcall(chat, "Found ore but could not put a warp plate down here.")
        setPhase("mining")
        return
    end

    chat(string.format(
        "Allthemodium found - warp plate is down%s. I will wait here until you arrive.",
        sent and " and the warp stone is on its way to you" or ""))
    print("pausing functionality")
    print("Press Enter on this turtle to resume.")
    -- Deliberate indefinite wait: see the header note. The plate stays on the
    -- ground the whole time, which is the point - resuming on a timer would
    -- take the warp point away before anyone could use it.
    io.read()
...
local function seek(x,y,z)
    ...
    -- The two y adjustments below are carried over verbatim from the version
    -- that was running on the rig, INCLUDING `y = 1 + 1`. That almost certainly
    -- means `y = y + 1`: as written it pins y to 2, so a downward approach
    -- always descends exactly two blocks no matter how far the ore actually is.
    -- It is preserved rather than corrected because this pass is about item
...
while true do
    if state.cycles > 3 then
        refuel()
        --warpPlate() --allthemodium patch: seek() already offers a warp point
        --            at every ore, so the timed one would only interrupt mining.
        state.cycles = 0
    end
